...
% Repo-relative paths (this file lives in <repo>/Utilities/)
scriptDir = fileparts(mfilename('fullpath'));
repoRoot  = fileparts(scriptDir);
addpath(genpath(repoRoot));
dataFile  = fullfile(repoRoot, 'Datasets', 'South_Lombardy_sorted_data.mat');
outputDir = fullfile(repoRoot, 'RealDataExperiment', 'outputs', 'GP_realDataExperiment');
if ~exist(outputDir, 'dir'); mkdir(outputDir); end

if ~exist(dataFile, 'file')
    error('File dati non trovato: %s', dataFile);
end
S = load(dataFile);
data = S.sorted_data;
...
save(fullfile(outputDir, 'SR_Baseline_Results.mat'), 'results', 'ResultsSR');
writetable(results, fullfile(outputDir, 'SR_Baseline_Results.csv'));
fprintf('Saved: %s\n', fullfile(outputDir, 'SR_Baseline_Results.mat'));